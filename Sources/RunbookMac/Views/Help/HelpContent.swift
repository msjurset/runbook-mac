    ...
    var sections: [HelpSection] {
        ...
        case .scheduling:
            return [
                ...
                .code("runbook cron add my-runbook \"0 3 * * 0\"\nrunbook cron add report \"0 6 * * *\" --var type=daily --var path=/r/daily.csv\nrunbook cron add report \"0 7 1 * *\" --var type=monthly --var path=/r/monthly.csv\nrunbook cron list\nrunbook cron remove my-runbook              # all schedules\nrunbook cron remove report \"0 6 * * *\"     # just the daily"),
                .paragraph("--var (repeatable) bakes CLI variables into a schedule, so the same runbook can be installed under multiple schedules with different inputs — daily vs monthly, dev vs prod, fast vs deep — without forking the YAML."),
                .heading("Cron Syntax"),
                ...
                .heading("Multiple Schedules per Runbook"),
                .paragraph("Schedule the same runbook more than once and the Schedules view collapses them into a single card: one header (status dot, name, last-run badge, schedule count), one step pipeline at the bottom, and one sub-row per schedule in between. Each sub-row carries its own cron expression, human description, --var values (when set), next-run line, and edit/delete buttons. Runbooks with only one schedule keep the original card layout."),
                .paragraph("Common use case: a daily and a monthly variant of one report runbook. Schedule the runbook twice with different --var inputs (and optionally a `condition:` step in the YAML that branches on those vars). The runbook YAML stays single-source-of-truth; the schedule choice is what differs."),
                .heading("Expand / Collapse"),
                .paragraph("Click the runbook name (or its chevron) to collapse the row down to just the header — useful when several long step charts make the screen busy. Click again to expand. Editing a schedule force-expands the row so the form is visible. The collapsed/expanded state is per-row and not persisted across launches. For grouped (multi-schedule) cards, collapse hides every sub-row and the pipeline as a unit."),
                .heading("Step-Type Legend"),
                .paragraph("The shell / ssh / http / confirm color legend lives on the \"Next: ...\" line and is shown only while the mouse is hovering inside a row's content area. Hovering one row hides any other row's legend automatically — at most one is visible at a time, so the screen stays quiet by default and reveals the key only where you're focused."),
        ...
        case .running:
            return [
                ...
                .numbered([
                    ...
                    "Fill in any variable values, optionally toggle Dry Run",
                    "Click Run to start execution",
                    "Watch live output streaming in the bottom Console Tray",
                ]),
                .heading("Console Tray"),
                .paragraph("Runs are non-modal — they execute in a docked tray at the bottom of the main window so you can keep editing other runbooks, switch sidebar sections, or start additional runs concurrently while one's in flight."),
                .paragraph("The tray's header is a tab strip — each concurrent run gets one tab, equally distributed across the bar. Tabs show a status icon (running / succeeded / failed / cancelled) and the runbook name. Click a tab to switch which run's output is shown. Click the × on a tab to stop and dismiss that run atomically; the chevron on the right collapses the tray to a single status bar."),
                .paragraph("The output toolbar (above the streaming log) carries Find, Copy All, Save, plus a context-aware action: Stop while running, or a Dry checkbox + Retry once a session is terminal. The Dry checkbox defaults to whatever the previous run was, so the common case is one click to repeat exactly. Up to 5 finished sessions are retained in the tab strip until you dismiss them."),
                .heading("Stopping a Run"),
                .paragraph("Click Stop in the output toolbar (or press ⌘.) to cancel without losing the tab. Click the × on the tab itself to stop and dismiss in one motion."),
                .heading("Retrying"),
                .paragraph("On a finished session, the Retry button restarts the same tab in place — output cleared, elapsed counter reset, same vars and runbook. Toggle the inline Dry checkbox first to flip between real and dry run for the retry."),
                .heading("Dry Run"),
                .paragraph("The Run Confirm sheet that appears when you click Run includes a Dry Run checkbox. Dry-run sessions show a blue 'dry' badge on their tab. After a dry run completes you can flip the Dry checkbox next to Retry and click Retry to execute for real in the same tab — no need to relaunch from scratch."),
